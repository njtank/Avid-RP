...
TriggerClientEvent("avid-metaldetect:client:startdetecting", src)
end)

RegisterServerEvent('avid-metaldetecting:server:giveloot', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('avid-metaldetecting:server:cleanclump', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local metaldetectitem =  Config.LootRare[math.random(1, #  Config.LootRare)] 
	local metaldetectlow =  Config.Loot[math.random(1, #  Config.Loot)] 
...
RegisterServerEvent('avid-metaldetecting:server:buydetector', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent("avid-metaldetecting:server:sellloot", function(amount, item, value)
	local src = source 
	local Player = QBCore.Functions.GetPlayer(src)

...
AddEventHandler('onResourceStart', function(resource)
    ...
    TriggerClientEvent('avid-weed:client:syncPlantList', -1)
...
RegisterNetEvent('avid-weed:server:destroyPlant', function(args)
    ...
    if DoesEntityExist(entity) then
        ...
        TriggerClientEvent('avid-weed:client:startPlantFire', -1, plants[entity].coords, stage)
...
RegisterNetEvent('avid-weed:server:harvestPlant', function(entity)
...
RegisterNetEvent('avid-weed:server:plantTakeCare', function(entity, item)
...
RegisterNetEvent('avid-weed:server:giveWater', function(entity, item)
    if not plants[entity] then return end
    local src = source
    local player = it.getPlayer(src)
    if not player then return end
...
RegisterNetEvent('avid-weed:server:giveFertilizer', function(entity, item)
    if not plants[entity] then return end
    local src = source
    local player = it.getPlayer(src)
    if not player then return end
...
RegisterNetEvent('avid-weed:server:createNewPlant', function(coords, plantItem, zone, metadata)
    local src = source
    local player = it.getPlayer(src)
...
-- Callbacks
lib.callback.register('avid-weed:server:getPlantData', function(source, netId)
...
lib.callback.register('avid-weed:server:getPlantDataWithId', function(source, plantId)
...
lib.callback.register('avid-weed:server:getPlantsOwned', function(source)
    local src = source
...
lib.callback.register('avid-weed:server:getPlants', function(source)
    return plants
end)

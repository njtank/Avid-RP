    ...
    if it.inventory == 'ox' then
        ...
        if not itemData then
            if Config.Debug then
                print('[avid-weed] The item ' .. item .. ' does not exist in the inventory.')
                return false
            end
        end
...
function it.createUsableItem(item, cb)
    if ConsumableItems[item] then print('[avid-weed] The item ' .. item .. ' is already registered as a consumable item. Skipping the registration of this item.') end
...
function it.getItemLabel(source, itemName)
    local itemLabel
    if it.inventory == 'ox' then
        itemLabel = lib.callback.await('avid-weed:client:getItemLabel', source, itemName)
    elseif it.core == 'qb-core' then
...
lib.callback.register('avid-weed:hasItem', function(source, item, amount)
...
lib.callback.register('avid-weed:getItemCount', function(source, item)
    local itemCount = it.getItemCount(source, item)
    return itemCount
end)

lib.callback.register('avid-weed:server:getItemLabel', function(source, itemName)
    local itemLabel = CoreObject.GetItemLabel(itemName)
    return itemLabel
end)

RegisterNetEvent('avid-weed:toggleItem', function(toggle, name, amount, metadata)
    local source = source

                    ...
                    options = {
                        {
                            ...
                            action = function (entity)
                                TriggerEvent('avid-weed:client:checkPlant', {entity = entity})
                            end
                        }
                    },
                    distance = 1.5,
        ...
        for k, v in pairs(Config.PlantTypes) do
            for _, plant in pairs(v) do
                exports.ox_target:addModel(plant[1], {
                    {
                        label = _U('TARGET__PLANT__LABEL'),
                        name = 'avid-weed-check-plant',
                        icon = 'fas fa-eye',
                        onSelect = function(data)
                            TriggerEvent('avid-weed:client:checkPlant', {entity = data.entity})
                        end,
                        distance = 1.5
                    }
                })
                        ...
                        options = {
                            {
                                ...
                                action = function (entity)
                                    TriggerEvent('avid-weed:client:showDealerMenu', k)
                                end
                            }
                        },
                        distance = 1.5,
                        ...
                        options = {
                            {
                                ...
                                action = function (entity)
                                    TriggerEvent('avid-weed:client:useTable', {entity = entity, type = k})
                                end
                            }
                        },
                        distance = 1.5,
            ...
            for k, v in pairs(Config.ProcessingTables) do
                if v.model ~= nil then
                    exports.ox_target:addModel(v.model, {
                        {
                            label = _U('TARGET__TABLE__LABEL'),
                            name = 'avid-weed-use-table',
                            icon = 'fas fa-eye',
                            onSelect = function(data)
                                TriggerEvent('avid-weed:client:useTable', {entity = data.entity})
                            end,
                            distance = 1.5
                        }
                    })
            ...
            options = {
                {
                    ...
                    icon = 'fas fa-comment',
                    action = function(entity)
                        TriggerEvent('avid-weed:client:checkSellOffer', entity)
                    end,
        ...
        exports.ox_target:addGlobalPed({
            {
                ...
                icon = 'fas fa-comment',
                onSelect = function(data)
                    TriggerEvent('avid-weed:client:checkSellOffer', data.entity)
                end,
...
RemoveSellTarget = function()
    ...
    elseif Config.Target == 'ox_target' then
        -- Check if ox target is running
        if not exports.ox_target then return end
        exports.ox_target:removeGlobalPed('avid-weed-sell')
    end
end
...
AddEventHandler('onResourceStop', function(resource)
    ...
    elseif Config.Target == 'ox_target' then
        ...
        for k, v in pairs(Config.PlantTypes) do
            for _, plant in pairs(v) do
                exports.ox_target:removeModel(plant[1], 'avid-weed-check-plant')
            end
        end
        for k, v in pairs(Config.ProcessingTables) do
            if v.model ~= nil then
                exports.ox_target:removeModel(v.model, 'avid-weed-use-table')
            end
        end
    end

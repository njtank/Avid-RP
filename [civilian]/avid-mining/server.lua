...
function ProcessCheck(input)
    ...
    if buckets >= sum then 
        ...
        TriggerClientEvent('avid-mining:ResetisProcess', source)
        return false 
    end 
end
    ...
    if GemCheck() == true then 
        local success = Inventory:CanCarryItem(source, 'water', 1)
            if success then 
                ...
                TriggerClientEvent('avid-mining:ResetisBreakdown', source)
...
function CanMine(reward)
    ...
    if bucketCheck then 
        ...
        if success then 
            ...
            TriggerClientEvent('avid-mining:ResetisMining', source)
            NotEnoughSpace()
        end 
    else 
        TriggerClientEvent('avid-mining:ResetisMining', source)

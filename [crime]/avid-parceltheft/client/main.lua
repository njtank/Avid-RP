...
local HoldBox = function()
    ...
    if packageAmount and packageAmount > 0 then
        if not holdingBox then
            ...
            CarryAnimation()
            SD.LoadModel('prop_cs_box_clothes')
            Parcel = CreateObject('prop_cs_box_clothes', 0, 0, 0, true, true, true)
        ...
        options = {
            {
                action = function()
                    ...
                    TriggerServerEvent('avid-parceltheft:server:MarkPropTaken', k)
...
local RequestLocationsFromServer = function()
    SD.Callback("avid-parceltheft:server:GetLocations", false, function(locations)
        if locations then
...
RegisterNetEvent('avid-parceltheft:client:RemoveProp', function(propId)
...
RegisterNetEvent('avid-parceltheft:client:ResetProp', function(propId)
    if localLocations[propId] then
...
RegisterNetEvent('avid-parceltheft:client:openBox', function()
    ...
    function()
        ...
        TriggerServerEvent('avid-parceltheft:server:deleteParcel')
    end,
    function()
        ClearPedTasks(ped)

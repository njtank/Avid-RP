document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (event.key === "Escape") {
            if (menu) {
                fetch('https://avid_pausemenuv2-restyle/continue', {
                    method: 'POST',
                    body: JSON.stringify({})
                })
            } 
    ...
    document.getElementById('map-btn').addEventListener('click', () => {
        fetch('https://avid_pausemenuv2-restyle/map', {
            method: 'POST',
            body: JSON.stringify({})
        })
    });
    ...
    document.getElementById('settings-btn').addEventListener('click', () => {
        fetch('https://avid_pausemenuv2-restyle/settings', {
            method: 'POST',
            body: JSON.stringify({})
        })
    });
    ...
    document.getElementById('logout-btn').addEventListener('click', () => {
        fetch('https://avid_pausemenuv2-restyle/logout', {
            method: 'POST',
            body: JSON.stringify({})
        })
    });

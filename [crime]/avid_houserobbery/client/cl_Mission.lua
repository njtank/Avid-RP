...
if Config.HouseType == "OnlyMission" or Config.HouseType == "Both" then
                    ...
                    npctxtui = lib.zones.sphere({
                        ...
                        inside = function(self)
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('avid_houserobbery:localize')
                            end
                ...
                elseif Config.InteractionType == 'target' then
                    SetTimeout(1000, function()
                        ...
                        target:AddTargetEntity(NPC, {
                            options = {
                                {
                                    event = "avid_houserobbery:localize",
                                    icon = "fas fa-store",
                                    label = locale("GetJob"),
                                },
                            },
    ...
    AddEventHandler("avid_houserobbery:localize", function()
        if not finding then
            ...
            house = FoundHouse
            TriggerServerEvent('avid_houserobbery:localize', 'newmission', FoundHouse)
            StartMission(house)
            FoundHouse = false
            FoundChance = false
        else
            TriggerServerEvent('avid_houserobbery:localize', 'cancelmission', FoundHouse)
    ...
    AddEventHandler("avid_houserobbery:missionoccupied", function(house)
    ...
    StartMission = function(house)
        if not finding then
            ...
            TriggerServerEvent('avid_houserobbery:missionoccupied', house)
            finding = true
            ...
            TriggerServerEvent("avid_houserobbery:missionsync", random_house)
            CreateThread(function()
                local sleep = 500
                while true do
                    local Coords = GetEntityCoords(PlayerPedId())
    ...
    RegisterNetEvent("avid_houserobbery:gethelp")
    AddEventHandler("avid_houserobbery:gethelp", function()
        if finding then
    ...
    AddEventHandler("avid_houserobbery:missionsync", function(houseid)
        ...
        elseif Config.InteractionType == 'target' and Config.HouseType == "OnlyMission" then
            SetTimeout(1000, function()
                target = Target()
                if not missionzones[houseid] then
                    missionzones[houseid] = target:AddCircleZone("avid_houserobbery_RobHouse_".. houseid, Config.HousesToRob[houseid].Coords.xyz, 1.5, {
                        name = "avid_houserobbery_RobHouse_".. houseid,
...
RegisterNetEvent("avid_houserobbery:missionoccupiedreset")
AddEventHandler("avid_houserobbery:missionoccupiedreset", function()
    MissionOccupied = {}
end)
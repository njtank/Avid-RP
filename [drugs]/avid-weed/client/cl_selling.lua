...
RegisterNetEvent('avid-weed:client:checkSellOffer', function(entity)
	local copsAmount = lib.callback.await('avid-weed:server:getCopsAmount', false)
	...
	TriggerEvent('avid-weed:client:showSellMenu', {item = sellItemData.item, price = sellItemData.price, amount = sellAmount, entity = entity})
	SetTimeout(Config.SellSettings['sellTimeout']*1000, function()
		...
		if lib.getOpenContextMenu() ~= nil then
			...
			if currentMenu == 'avid-weed-sell-menu' then
...
RegisterNetEvent('avid-weed:client:salesInitiate', function(cad)
	AddSoldPed(cad.tped)
	if cad.type == 'close' then
		...
		TriggerServerEvent('avid-weed:server:initiatedrug', cad)
		SetPedAsNoLongerNeeded(cad.tped)
	end
end)
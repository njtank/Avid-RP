...
local function pickPocket(data)
    ...
    then
        TriggerServerEvent('avid-pickpocket:server:pickPocketNpc', NetworkGetNetworkIdFromEntity(data.entity))
...
lib.callback.register('avid-pickpocket:client:getZoneName', function(coords)
    return GetNameOfZone(coords.x, coords.y, coords.z)
end)

RegisterNetEvent('avid-pickpocket:client:failedPickpocket', function(netId)
    if GetInvokingResource() then return end
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not DoesEntityExist(entity) or IsEntityDead(entity) then return end
    ClearPedTasksImmediately(entity)
    GiveWeaponToPed(entity, `WEAPON_SWITCHBLADE`, 255, false, false)

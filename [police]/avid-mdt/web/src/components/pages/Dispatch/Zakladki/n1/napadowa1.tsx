...
const Napadowa1:React.FC = () => {
    ...
    const setSV = (self: boolean) => {
        ...
        fetchNui('setNapadowaSV', {zakladkaType: 'napadowa1', sv: sv})
    }
    ...
    const setNego = (self: boolean) => {
        ...
        fetchNui('setNapadowaNEGO', {zakladkaType: 'napadowa1', nego: nego})
    }

    const clearnapadowa = () => {
        fetchNui("clearNapadowa", {zakladkaType: 'napadowa1'})
    }
    
    const removePatrol = () => {
        if (clickedPatrol == null) return;
        setRemoveModal(false)
        fetchNui("removeUnit", {zakladkaType: 'napadowa1', index: clickedPatrol + 1})
    ...
    const submitEdit = () => {
        ...
        if(editedOfficer1 != '' && editedOfficer2 != '' && editedOfficer3 != '' && editedOfficer4 != '' && editedOfficer5 != ''){
            if (clickedPatrol === null) {
                ...
                fetchNui("createUnit", {zakladkaType: 'napadowa1', unit: editedUnit, vehicle: editedVehicle, officers: officers, patrolUnit: pu})
            } else {
                var officers = {
                    officer1: editedOfficer1,
                    officer2: editedOfficer2,
                ...
                fetchNui("editUnit", {zakladkaType: 'napadowa1', index: clickedPatrol + 1, unit: editedUnit, vehicle: editedVehicle, officers: officers, patrolUnit: pu})
            }
        }
        setClickedPatrolUnit(null)
    }
    ...
    const ZadaniaChange = (z: string) => {
        fetchNui("updateZadaniaAkcyjna", {zakladkaType: 'napadowa1', text: z})
    }

    const NotatkiChange = (z: string) => {
        fetchNui("updateNotatkiAkcyjna", {zakladkaType: 'napadowa1', text: z})
    }

    const RabunekChange = (z: string) => {
        fetchNui("updateRabunekAkcyjna", {zakladkaType: 'napadowa1', text: z})
    }
    ...
    return (
        <>
            {removeModal && <ConfirmModal label= {Locale['DELETE_CHASE_UNIT']} text={"Czy na pewno chcesz usunąć " + (clickedPatrolUnit !== null && clickedPatrolUnit) + "?"} submitLabel={Locale['DELETE']} cancelLabel={Locale['CANCEL']} onSubmit={removePatrol} onCancel={() => setRemoveModal(false)} />}
            {editModal && <>

...
QBCore.Functions.CreateCallback('avid-graffiti:server:getGraffitiData', function(source, cb)
...
RegisterServerEvent('avid-graffiti:client:addServerGraffiti', function(model, coords, rotation)
    local source = source
...
RegisterServerEvent('avid-graffiti:server:removeServerGraffitiByKey', function(key)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('avid-graffiti:server:graffitiShop', function(data)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('avid-graffiti:server:removeServerItem', function(item, amount, slot)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem('spraycan', function(source, item)
    ...
    if Player and isLoaded then
        if item.info then
            TriggerClientEvent('avid-graffiti:client:placeGraffiti', source, item.info.model, item.slot)
        else
...
QBCore.Functions.CreateUseableItem('sprayremover', function(source, item)
    ...
    if Player and isLoaded then
        TriggerClientEvent('avid-graffiti:client:removeClosestGraffiti', source, item.slot)
    end
end)

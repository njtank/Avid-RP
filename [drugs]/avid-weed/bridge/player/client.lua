...
function it.getCitizenId()
    local citizenId = lib.callback.await('avid-weed:getCitizenId', false)
    return citizenId
end
...
function it.getPlayerName()
    ...
    elseif it.core == 'esx' then
        return lib.callback.await('avid-weed:getPlayerName', false)
    end
end

function it.getLicences()
    local licences = lib.callback.await('avid-weed:getLicences', false)
    return licences
end

function it.getLicence(licenseType)
    local licence = lib.callback.await('avid-weed:getLicence', false, licenseType)
    return licence
end
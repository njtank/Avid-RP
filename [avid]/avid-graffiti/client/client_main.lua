    ...
    exports['ox_target']:addLocalEntity(npc, {
        {
            ...
            distance = 3.0,
            onSelect = function()
                TriggerEvent('avid-graffiti:client:graffitiShop')
            end
        }
    })
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    if not isLoaded then
        QBCore.Functions.TriggerCallback('avid-graffiti:server:getGraffitiData', function(data)
...
RegisterNetEvent('avid-graffiti:client:setGraffitiData', function(data)
    if isLoaded then
        for k,v in pairs(Config.Graffitis) do
            if v then
                if DoesEntityExist(v.entity) then
...
RegisterNetEvent('avid-graffiti:client:placeGraffiti', function(model, slot)
        ...
        if information then
            ...
            PlaceGraffiti(model, function(result, coords, rotation)
                if result then
                    ...
                    TriggerServerEvent('avid-graffiti:server:removeServerItem', 'spraycan', 1, slot)
                    ...
                    QBCore.Functions.Progressbar('spraying_on_wall', Lang:t('progressbar.spraying_on_wall'), 40000, false, false, {
                        ...
                        TriggerServerEvent('avid-graffiti:client:addServerGraffiti', model, coords, rotation)
                    end)
                end
            end)
        else
...
RegisterNetEvent('avid-graffiti:client:removeClosestGraffiti', function(slot)
    local ped = PlayerPedId()
    ...
    if isLoaded then
        if not graffiti then
            ...
            TriggerServerEvent('avid-graffiti:server:removeServerItem', 'sprayremover', 1, slot)
            ...
            QBCore.Functions.Progressbar('spraying_on_wall', Lang:t('progressbar.washing_the_wall'), 20000, false, false, {
                ...
                TriggerServerEvent('avid-graffiti:server:removeServerGraffitiByKey', tonumber(graffiti))
            end)
        end
    end
end)

RegisterNetEvent('avid-graffiti:client:graffitiShop', function()
    local PlayerData = QBCore.Functions.GetPlayerData()
    local graffitiMenu = {}
    ...
    if isLoaded then
        ...
        for k,v in pairs(Config.Sprays) do
            graffitiMenu[#graffitiMenu + 1] = {
                ...
                params = {
                    isServer = true,
                    event = 'avid-graffiti:server:graffitiShop',
                    args = {
                        model = k,
                        name = v.name,
                        price = v.price 

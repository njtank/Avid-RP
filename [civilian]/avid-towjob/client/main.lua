...
CreateThread(function()
    ...
    -- Create the ped
    local towped = CreatePed(0, Config.PedHash, Config.PedPos.x, Config.PedPos.y, Config.PedPos.z - 1.0, Config.PedPos.w, false, false)
    ...
    -- Add ox_target options as a box zone
    exports['ox_target']:addBoxZone({
        coords = vec4(401.99, -1626.29, 29.29, 237.04),
        size = vec3(2.0, 2.0, 2.0), -- Adjust size as needed
        rotation = 237.04,
        options = {
            {
...
local function MenuGarage()
    ...
    lib.showContext('tow_garage_menu')
end
...
RegisterNetEvent('avid-towjob:client:ToggleNpc', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            ...
            local modelName = GetDisplayNameFromVehicleModel(selectedVeh)
                ...
                if not IsPedInAnyVehicle(PlayerPedId()) then
                    if vehicle ~= targetVehicle then
                        ...
                        if #(towPos - targetPos) < 11.0 then
                            QBCore.Functions.Progressbar("towing_vehicle", "Attaching to tow truck", 12000, false, true, {
                ...
                QBCore.Functions.Progressbar("untowing_vehicle", Lang:t('info.removing_vehicle'), 12000, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
...
AddEventHandler("an-tow:parkcar", function()
    local coords = vector3(403.72, -1628.14, 29.29)
    ...
    exports['ox_target']:addGlobalVehicle({
        {
            ...
            canInteract = function(entity)
                ...
                local flatbed = GetHashKey('flatbed')
                local slamtruck = GetHashKey('slamtruck')
                return GetEntityModel(oldtruck) == flatbed or GetEntityModel(oldtruck) == slamtruck
            end,
            --groups = {"tow", "mechanic", "hayes", "harmony"}
        }
    })
end)

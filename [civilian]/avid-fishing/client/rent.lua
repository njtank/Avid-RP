...
local function spawnBoat(boat)
    ...
    if closest then
        ...
        Framework.spawnVehicle(boat.model, coords, closest.spawn.w, function(vehicle)
            ...
            TriggerServerEvent('avid-fishing:registerBoat', NetworkGetNetworkIdFromEntity(vehicle))
        end)
    end
end
...
local function rentVehicle(index)
    ...
    local success = lib.callback.await('avid-fishing:rentVehicle', false, index)
...
local bind = lib.addKeybind({
    ...
    onReleased = function()
        ...
        local success = lib.callback.await('avid-fishing:returnVehicle', false, netId)

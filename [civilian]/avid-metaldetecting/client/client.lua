...
RegisterNetEvent("avid-metaldetect:client:metaldetect")
AddEventHandler("avid-metaldetect:client:metaldetect", function() 
...
RegisterNetEvent("avid-metaldetect:client:startdetecting")
AddEventHandler("avid-metaldetect:client:startdetecting", function()
	if metal then
		...
		metal = object
		TriggerEvent("avid-metaldetect:client:metaldetect")
	end
end)
...
if success then
	TriggerServerEvent('avid-metaldetecting:server:giveloot')
		...
		options = {
				{
					...
					onSelect = function()
						...
						TriggerServerEvent('avid-metaldetecting:server:cleanclump')
						ClearPedTasks(PlayerPedId())
						end)
					end,
				},
...
for k, v in pairs (Config.LootSell) do 
	metaldetectsales[#metaldetectsales + 1] = {
				   icon = Config.InvWeblink..QBCore.Shared.Items[v.name].image,
					title = v.label,
					event = "avid-metaldetecting:client:sellloot",
					args = {
						item = v.name,
						cost = v.price,
					}
                ...
                options = {
                    {
                        ...
                        action = function()
							...
							TriggerServerEvent("avid-metaldetecting:server:buydetector")
							ClearPedTasks(PlayerPedId())
							end)
						end	
                    },
...
RegisterNetEvent("avid-metaldetecting:client:sellloot", function(data) local dialog
	...
	local dialog = exports.ox_lib:inputDialog(QBCore.Shared.Items[data.item].label,   {
		...
		TriggerServerEvent("avid-metaldetecting:server:sellloot", dialog[1], data.item, data.cost)
end)

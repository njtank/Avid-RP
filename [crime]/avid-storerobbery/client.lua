...
local function Alert(id)
    ...
    cfg.alerted = true
    TriggerServerEvent("avid-storerobbery:server:registerAlert", id, true)
...
local function RobRegistar()
    ...
    TriggerServerEvent("avid-storerobbery:server:setUse", CurrentStore, RegZoneID, true)
    ...
    if lib.progressCircle({
        ...
        local success = exports['avid-minigames']:MineSweep(prize, 10, 3, "left")
        if success then
        ...
        TriggerServerEvent("avid-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
    else
        TaskPlayAnim(ped, anim, "exit", 8.0, 8.0, -1, 0, 1.0, false, false, false)
        TriggerServerEvent("avid-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
    end
...
local function SearchCombination(storeid, sid)
    ...
    if searchLoc.iscomputer then
        ...
        if success then
            ...
            if canGet then
                ...
                SetTimeout(1000 * TimeToWait, function()
                    lib.callback.await("avid-storerobbery:server:combination", false, storeid, sid, true)
                    Functions.Notify("You got the combination key")
                end)
            else
                lib.callback.await("avid-storerobbery:server:combination", false, storeid, sid, false)
        ...
        if lib.progressCircle({
            ...
            lib.callback.await("avid-storerobbery:server:combination", false, storeid, sid, canGet)
            if canGet then
                Functions.Notify("You got the combination key")
            else
...
local function SetupStore(id)
    ...
    local function Hack()
        ...
        if cfg.cooldown then return end
        local success = exports['avid-minigames']:MemoryCard()
        TriggerServerEvent("avid-storerobbery:server:setHackUse", id, true)
        if success then
            TriggerServerEvent("ran-houserobbery:server:setHackedState", id, true)
            Wait(500)
            Alert(id)
        end
        TriggerServerEvent("avid-storerobbery:server:setHackUse", id, false)
    end
    local function OpenPinPrompt()
        ...
        if num == cfg.combination then
            lib.callback.await("avid-storerobbery:server:setSafeState", false, CurrentStore, true)
...
lib.callback.register("avid-storerobbery:client:openConfigContext", function()
    return ConfigContext()
end)

lib.callback.register("avid-storerobbery:client:resetStore", function()
    if not CurrentStore then return end
    local cfg = Config.Store[CurrentStore]
    if not cfg then return end
...
RegisterNetEvent("avid-storerobbery:client:setConfigs", function(cfg)
    Config.Store = cfg
end)

RegisterNetEvent("avid-storerobbery:client:setStoreConfig", function(id, cfg)

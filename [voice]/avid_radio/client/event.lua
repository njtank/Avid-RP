AddEventHandler('onResourceStart', function(resource)
    if GetCurrentResourceName() == resource then
        if Shared.Core and Framework.core.playerLoaded() then
            ...
            TriggerServerEvent('avid_radio:server:createdefaultjammer')
        end
    end
end)
...
RegisterNetEvent('avid_radio:client:use', function(slot)
    ...
    local battery, radioId = lib.callback.await('avid_radio:server:getradiodata', false, slot)
...
RegisterNetEvent('avid_radio:client:usejammer', function()
    ...
    TriggerServerEvent('avid_radio:server:spawnobject', {
...
RegisterNetEvent('avid_radio:client:syncobject', function(data)
...
RegisterNetEvent('avid_radio:client:changeJammerRange', function(id, range)
...
RegisterNetEvent('avid_radio:client:removeallowedchannel', function(id, allowedChannels)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
...
RegisterNetEvent('avid_radio:client:addallowedchannel', function(id, allowedChannels)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.allowedChannels = allowedChannels
...
RegisterNetEvent('avid_radio:client:togglejammer', function(id, state)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.enable = state
...
RegisterNetEvent('avid_radio:client:removejammer', function(id)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.zone:remove()
...
RegisterNetEvent('avid_radio:client:remove', function()
...
RegisterNetEvent('avid_radio:client:radioListUpdate', function(players, channel)
    if Radio.RadioChannel == channel then
        Radio:SendSvelteMessage("updateRadioList", players)
    end
end)

RegisterNetEvent('avid_radio:client:nocharge', function()
    Radio:leaveradio()
end)

RegisterNetEvent("avid_radio:client:recharge", function()
    TriggerServerEvent('avid_radio:server:rechargeBattery')
end)
...
RegisterNetEvent('bl_bridge:client:playerLoaded', function()
    ...
    Radio:Init()
    TriggerServerEvent('avid_radio:server:createdefaultjammer')
    local jammer = lib.callback.await('avid_radio:server:getjammer', false)
    for i = 1, #jammer do
        jammer[i].entity = NetworkGetNetworkIdFromEntity(jammer[i].entity)
        TriggerEvent('avid_radio:client:syncobject', jammer[i])
    end
end)
    ...
    if event == "CEventNetworkEntityDamage" then
        ...
        if data[4] and NetworkGetPlayerIndexFromPed(data[1]) == cache.playerId and IsEntityDead(cache.ped) then
            if Radio.usingRadio then
                TriggerEvent('avid_radio:client:remove')
            end

...
local plantTherminte = function()
    ...
    TriggerServerEvent('avid-jewelry:server:removeThermiteItem')
    ...
    TriggerServerEvent('avid-jewelry:server:toggleDoors', false)
    TriggerServerEvent('avid-jewelry:server:setStoreHit', true)
end
...
local smashVitrine = function(data)
    ...
    TriggerServerEvent('avid-jewelry:server:setVitrineBusy', data.vitrine.id, true)
    ...
    TriggerServerEvent('avid-jewelry:server:setVitrineState', data.vitrine.id, true)
    ...
    if _Progress(locale('smash_progress'), 10000, {
        ...
        TriggerServerEvent('avid-jewelry:server:vitrineReward', data.vitrine.id)
        TriggerServerEvent('avid-jewelry:server:setVitrineBusy', data.vitrine.id, false)
    end
end
...
RegisterNetEvent('avid-jewelry:client:setVitrines', function(vitrines)
    setVitrines(vitrines)
end)

RegisterNetEvent('avid-jewelry:client:setVitrineState', function(id, value)
...
RegisterNetEvent('avid-jewelry:client:setVitrineBusy', function(id, value)
    if type(id) == 'number' then
        local vitrine = lib.array.find(Vitrines, function(v)
            return v.id == id
        end)
...
RegisterNetEvent('avid-jewelry:client:startThermite', function()
    local isOnCooldown, timeLeft = lib.callback.await('avid-jewelry:server:isOnCooldown')
    if isOnCooldown then
        _Notification(locale('notification_on_cooldown', timeLeft), 'error')
        return
    end

    if lib.callback.await('avid-jewelry:server:isStoreHit', false) then
        _Notification(locale('notification_store_is_hit'), 'error')
        return
    end

    if not lib.callback.await('avid-jewelry:server:isPoliceCountHighEnough', false) then
        _Notification(locale('notification_not_enough_police'), 'error')
        return
    end

    if not lib.callback.await('avid-jewelry:server:checkThermiteItem', false) then
...
Citizen.CreateThread(function()
    setVitrines(lib.callback.await('avid-jewelry:server:getVitrinesArray', false))
end)
    ...
    options = {
        {
            ...
            event = 'avid-jewelry:client:startThermite'
        }
    }
})
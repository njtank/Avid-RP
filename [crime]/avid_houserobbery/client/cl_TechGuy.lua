...
RegisterNetEvent("avid_houserobbery:Shop:menus", function()
    ...
    if Config.Context == "ox_lib" then
        for _, v in pairs(Config.Shop.Items) do
            ...
            options[v.label].event = 'avid_houserobbery:Shop:progress'
    ...
    elseif Config.Context == "qbcore" then
        ...
        for _, v in pairs(Config.Shop.Items) do
            options[#options + 1] = {
                ...
                params = {
                    event = 'avid_houserobbery:Shop:progress',
...
RegisterNetEvent("avid_houserobbery:Shop:progress")
AddEventHandler("avid_houserobbery:Shop:progress", function(data)
    ...
    if amount then
        if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
            lib.callback('avid_houserobbery:Shop:getitem', false, function(value)
                if value then
                    ...
                    TSE("avid_houserobbery:Shop:giveitems", data.item, data.price, amount)
                else
                    ...
                    npctxtui = lib.zones.sphere({
                        ...
                        inside = function(self)
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('avid_houserobbery:Shop:menus')
                            end
                ...
                elseif Config.InteractionType == 'target' then
                    ...
                    target:AddTargetEntity(houserobberyNPC, {
                        options = {
                            {
                                type = "client",
                                event = "avid_houserobbery:Shop:menus",
                                icon = "fas fa-laptop",
                                label = locale("shop")
                            },
                        },

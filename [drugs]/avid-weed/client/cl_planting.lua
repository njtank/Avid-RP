...
local plantSeed = function(ped, plant, plantInfos, plantItem, coords, metadata)
    ...
    local plants = lib.callback.await('avid-weed:server:getPlants', false)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:createNewPlant', coords, plantItem, zone, metadata)
        ClearPedTasks(ped)
...
-- Events
RegisterNetEvent('avid-weed:client:useSeed', function(plantItem, metadata)
    ...
    local ownedPlants = lib.callback.await('avid-weed:server:getPlantsOwned', false)
...
RegisterNetEvent('avid-weed:client:checkPlant', function(data)
    local netId = NetworkGetNetworkIdFromEntity(data.entity)
    lib.callback('avid-weed:server:getPlantData', false, function(result)
        if not result then return end
        if Config.Debug then lib.print.info('Checking Data:', result) end -- DEBUG
        -- Find event in client/cl_menus.lua
        TriggerEvent('avid-weed:client:showPlantMenu', result)
    end, netId)
end)

RegisterNetEvent('avid-weed:client:harvestPlant', function(args)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:harvestPlant', entity)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
local giveWater = function(args)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:plantTakeCare', entity, item)
        ClearPedTasks(ped)
...
RegisterNetEvent('avid-weed:client:useItem', function (args)
    local item = args.item
...
RegisterNetEvent('avid-weed:client:destroyPlant', function(args)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:destroyPlant', {entity = entity})
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
RegisterNetEvent('avid-weed:client:startPlantFire', function(coords)
    local ped = PlayerPedId()

...
const FilesCitizen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const enterClicked = (event: KeyboardEvent) => {
            ...
            if (document?.activeElement?.id !== 'searchcit') return;
            searchCitizen()
        }
        ...
        <div className="files-container" style={{padding: (currentResult && currentResult.type == "citizen") && "0 36px"}}>
            <div className="files" style={{width: (currentResult && currentResult.type == "citizen") && "1004px"}}>
                {currentResult === null && <>
                    ...
                    <TitleBlock text={Locale['FILES_SEARCHER'] + (Locale['FILES_SEARCH_FOR_CITIZEN'])}/>
                    <div className="searcher">
                        <input placeholder={Locale['FILES_SEARCHER'] + (Locale['FILES_SEARCH_FOR_CITIZEN']) + '...'} id="searchcit" ref={search}/>
                        <div className="closest" onClick={searchCitizen}>
                    ...
                    {results.length > 0 && 
                        <>
                            <TitleBlock text={Locale['FILES_SEARCH_FOR_CITIZEN']}/>
                                ...
                                <div className="results-results">
                                    <table>
                                        {results.map((value, index) => (
                                            <tr key={index} className="results-row" onClick={() => setData("citizen", value.identifier)}>
                ...
                {currentResult && <CitizenData data={currentResult} refresh={refreshData}/>}
            </div>
        </div>
    )
}

export default FilesCitizen
if IsDuplicityVersion() then
    ...
    function UpdatePlayer(data, remove, source)
        ...
        if deactivePlayers[source] then
            datas[source] = {}
            TriggerClientEvent("avid-backitems:updatePlayers", -1, {}, source)
            return
        end
        if isOxInventory then
            ...
            TriggerClientEvent("avid-backitems:updatePlayers", -1, weapons, xPlayer)
        elseif isQb then
            local source = source
            local xPlayer = QBCore.Functions.GetPlayer(source)
            local weapons = GetWeaponsFromPlayer(xPlayer)
            datas[xPlayer.PlayerData.source] = weapons
            TriggerClientEvent("avid-backitems:updatePlayers", -1, weapons, xPlayer.PlayerData.source)
        elseif isESX then
            local source = source
            local xPlayer = ESX.GetPlayerFromId(source)
            local weapons = GetWeaponsFromPlayer(xPlayer)
            datas[xPlayer.source] = weapons
            TriggerClientEvent("avid-backitems:updatePlayers", -1, weapons, xPlayer.source)
        else
            local source = source
            datas[source] = data
            TriggerClientEvent("avid-backitems:updatePlayers", -1, data, source)
        end
    end
    ...
    AddEventHandler("QBCore:Server:PlayerLoaded", function(xPlayer)
        if isOxInventory then
            ...
            local weapons = GetWeaponsFromPlayer(xPlayer)
            datas[xPlayer.PlayerData.source] = weapons
            TriggerClientEvent("avid-backitems:updatePlayers", -1, datas)
        end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerClientEvent("avid-backitems:updatePlayers", -1, datas)
        TriggerClientEvent("avid-backitems:forceStart", -1)
    end)
else
    local isQb = GetResourceState('qb-core') == 'started'
    local isESX = GetResourceState('es_extended') == 'started'
    ...
    elseif not isQb then
        ...
        Citizen.CreateThread(function ()
            ...
            while true do
                ...
                TriggerServerEvent("avid-backitems:updatePlayer", weapons)
                Wait(1000)
            end
        end)
    end
    ...
    RegisterNetEvent("inventory:client:UpdatePlayerInventory", function()
        TriggerServerEvent("avid-backitems:updatePlayer")
    end)
    ...
    RegisterNetEvent("avid-backitems:updatePlayers", function(data, playerId)
        if playerId then
    ...
    RegisterNetEvent("avid-backitems:forceStart", function()
        IsActive = true

RegisterNetEvent('avid-weed:client:showAdminAlertBox', function(args)
    ...
    if alert == 'confirm' then
        ...
        lib.setClipboard('add_ace identifier.'..userLicense..' avid-weed allow #'..username..' License')
        ShowNotification(nil, _U('NOTIFICATION__COPY__CLIPBOARD'):format('User License'), "success")
    end
end)

RegisterNetEvent('avid-weed:client:addAllAdminBlips', function(args)
...
RegisterNetEvent('avid-weed:client:addAdminBlip', function(args)
    local id = args.id
...
RegisterNetEvent('avid-weed:client:removeAllAdminBlips', function(args)
    local type = args.type
...
RegisterNetEvent('avid-weed:client:generatePlantListMenu', function()
    ...
    TriggerEvent('avid-weed:client:showPlantListMenu', {plantList = plantList})
end)

RegisterNetEvent('avid-weed:client:generateTableListMenu', function()
    local currentCoords = GetEntityCoords(PlayerPedId())
    local allTables = lib.callback.await('avid-weed:server:getTables', false)
    ...
    TriggerEvent('avid-weed:client:showTableListMenu', {tableList = tableList})
end)


RegisterNetEvent('avid-weed:client:showGroundHash', function()

...
local function SetupStore()
    TriggerClientEvent("avid-storerobbery:client:setConfigs", -1, Config.Store)
end
...
local function Cooldown(storeid)
    ...
    TriggerClientEvent("avid-storerobbery:client:setStoreConfig", -1, storeid, cfg)
end
...
RegisterNetEvent("avid-storerobbery:server:getConfig", function()
    local src = source
    TriggerClientEvent("avid-storerobbery:client:setConfigs", src, Config.Store)
end)

RegisterNetEvent("avid-storerobbery:server:setUse", function(storeid, regid, status)
    ...
    TriggerClientEvent("avid-storerobbery:client:setStoreConfig", -1, storeid, storeConfig)
end)

RegisterNetEvent("avid-storerobbery:server:setHackUse", function(storeid, status)
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
RegisterNetEvent("avid-storerobbery:server:registerAlert", function(storeid, status)
    local src = source
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
lib.callback.register("ran-houserobbery:server:getPrize", function(source, prize, storeid, regid)
    ...
    TriggerClientEvent("avid-storerobbery:client:setStoreConfig", -1, storeid, config)
    return true
end)

lib.callback.register("avid-storerobbery:server:combination", function(source, storeid, sid, status)
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
lib.callback.register("avid-storerobbery:server:setSafeState", function(_, storeid)
    local config = Config.Store[storeid]
    if not config then return end
...
lib.addCommand("reset-store", {
    ...
    local sc = lib.callback.await("avid-storerobbery:client:resetStore", source)
    if sc then
        Cooldown(sc)
    end
end)
...
lib.addCommand("get-store-config", {
    ...
    ---@type number | false
    local cb = lib.callback.await("avid-storerobbery:client:openConfigContext", source)
    if cb then

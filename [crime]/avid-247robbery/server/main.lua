...
--- @param source number
--- @return boolean
lib.callback.register('avid-247robbery:StartRobbery', function(source)
    if not source then
        EventLog('[main.lua]: avid-247robbery:StartRobbery: unable to retrieve source', 'error')
        return false
    end
    local source = source
    local identifier = GetIdentifier(source)
    if not identifier then
        EventLog('[main.lua]: avid-247robbery:StartRobbery: unable to retrieve player identifier', 'error')
        return false
    end
    local isNearRegister = IsPlayerNearby(source, 'registers')
    if not isNearRegister then
        EventLog('[main.lua]: avid-247robbery:StartRobbery: player not nearby any registers', 'error')
        return false
    end
    local hasRequiredItem = GetItemCount(source, Config.Registers.item) >= 1
    if not hasRequiredItem then
        TriggerClientEvent('avid-247robbery:Notify', source, Strings.Notify.missingItem, 'error')
        EventLog('[main.lua]: avid-247robbery:StartRobbery: player missing required item', 'error')
        return false
    end
    if Config.Police.require then
        local police = GetPoliceCount()
        if not police or police < Config.Police.count then
            TriggerClientEvent('avid-247robbery:Notify', source, Strings.Notify.notEnoughPolice, 'error')
            EventLog('[main.lua]: avid-247robbery:StartRobbery: not enough police to start robbery', 'error')
            return false
        end
    end
    if not CanPlayerRob(identifier) then
        TriggerClientEvent('avid-247robbery:Notify', source, Strings.Notify.registerCooldown, 'error')
        EventLog('[main.lua]: avid-247robbery:StartRobbery: cooldown is still active (for player or global)', 'error')
        return false
    end
...
RegisterNetEvent('avid-247robbery:DoesLockpickBreak', function()
    if not source then
        EventLog('[main.lua]: avid-247robbery:DoesLockpickBreak: unable to retrieve source', 'error')
        return
    end
    local source = source
    local identifier = GetIdentifier(source)
    if not identifier then
        EventLog('[main.lua]: avid-247robbery:DoesLockpickBreak: unable to retrieve player identifier', 'error')
        return
    end
    if not GlobalState.started then
        EventLog('[main.lua]: avid-247robbery:RegisterIncomplete: global state wasnt initiated', 'error')
        return
    end
    if not states[identifier] or states[identifier].state ~= 'in_progress' then
        EventLog('[main.lua]: avid-247robbery:DoesLockpickBreak: robbery wasnt initiated for player', 'error')
        return
    end
    local isNearRegister = IsPlayerNearby(source, 'registers')
    if not isNearRegister then
        EventLog('[main.lua]: avid-247robbery:DoesLockpickBreak: player not nearby any registers', 'error')
        return
    end
    if math.random(100) <= Config.Registers.breakChance then
        RemoveItem(source, Config.Registers.item, 1)
        TriggerClientEvent('avid-247robbery:Notify', source, Strings.Notify.lockpickBroke, 'error')
    end
    if GlobalState.started then GlobalState.started = false end
end)

-- Event to handle robbery completion and rewards
RegisterNetEvent('avid-247robbery:CompleteRegisterRobbery', function()
    if not source then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: unable to retrieve source', 'error')
        return
    end
    local source = source
    local identifier = GetIdentifier(source)
    if not identifier then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: unable to retrieve player identifier', 'error')
        return
    end
    local name = GetName(source)
    if not name then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: unable to retrieve player name', 'error')
        return
    end
    if not states[identifier] or states[identifier].state ~= 'in_progress' then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: robbery wasnt initiated for player', 'error')
        return
    end
    if not GlobalState.started then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: global state wasnt initiated', 'error')
        return
    end
    local isNearRegister = IsPlayerNearby(source, 'registers')
    if not isNearRegister then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: player not nearby any registers', 'error')
        return
    end
    local data = Config.Registers.reward
    if not data then
        EventLog('[main.lua]: avid-247robbery:CompleteRegisterRobbery: unable to retrieve config data', 'error')
        return
    end
...
-- Event to handle robbery completion and rewards
RegisterNetEvent('avid-247robbery:CompleteSafeRobbery', function()
    if not source then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: unable to retrieve source', 'error')
        return
    end
    local source = source
    local identifier = GetIdentifier(source)
    if not identifier then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: unable to retrieve player identifier', 'error')
        return
    end
    local name = GetName(source)
    if not name then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: unable to retrieve player name', 'error')
        return
    end
    if not GlobalState.started then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: global state wasnt initiated', 'error')
        return
    end
    if not states[identifier] or states[identifier].state ~= 'completed' then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: player has not completed the register robbery', 'error')
        return
    end
    local isNearSafe = IsPlayerNearby(source, 'safes')
    if not isNearSafe then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: player not nearby any safes', 'error')
        return
    end
    local data = Config.Safes.reward
    if not data then
        EventLog('[main.lua]: avid-247robbery:CompleteSafeRobbery: unable to retrieve safe reward config data', 'error')
        return
    end
    local quantity = math.random(data.min, data.max)
    if Config.Police.risk then
...
RegisterNetEvent('avid-247robbery:FailedRobbery', function()
    if not source then
        EventLog('[main.lua]: avid-247robbery:FailedRobbery: unable to retrieve source', 'error')
        return
    end
    local source = source
    local identifier = GetIdentifier(source)
    if not identifier then
        EventLog('[main.lua]: avid-247robbery:FailedRobbery: unable to retrieve player identifier', 'error')
        return
    end
    local name = GetName(source)
    if not name then
        EventLog('[main.lua]: avid-247robbery:FailedRobbery: unable to retrieve player name', 'error')
        return
    end
    if not GlobalState.started then
        EventLog('[main.lua]: avid-247robbery:FailedRobbery: global state wasnt initiated', 'error')
        return
    end
    if not states[identifier] or states[identifier].state ~= 'completed' then
        EventLog('[main.lua]: avid-247robbery:FailedRobbery: robbery wasnt initiated for player', 'error')
        return
    end

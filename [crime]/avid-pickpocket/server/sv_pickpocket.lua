...
RegisterNetEvent('avid-pickpocket:server:pickPocketNpc', function(netId)
    local src = source
    ...
    local zoneId = lib.callback.await('avid-pickpocket:client:getZoneName', src, pos)
    ...
    if math.random() > Server.Luck then
        ...
        TriggerClientEvent('avid-pickpocket:client:failedPickpocket', src, netId)
    end

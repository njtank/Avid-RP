...
lib.callback.register('avid-weed:server:getDealerPosition', function(_, dealerID)
...
lib.callback.register('avid-weed:server:getDealerItemData', function(_, dealerID, item)
    if not dealers[dealerID] then
        return
    end
...
RegisterNetEvent('avid-weed:server:buyDealerItem', function(dealerID, item, amount, total)
    local src = source

...
MySQL.ready(function()
    local data = MySQL.query.await('SELECT * FROM avid-fishing')
...
local function save()
    local query = 'UPDATE avid-fishing SET xp = ? WHERE user_identifier = ?'
    local parameters = {}
    local size = 0
...
local function createPlayer(identifier)
    ...
    MySQL.insert.await('INSERT INTO avid-fishing (user_identifier, xp) VALUES(?, ?)', { identifier, levels[identifier] })
end

lib.callback.register('avid-fishing:getLevel', function(source)
...
function AddPlayerLevel(player, amount)
    ...
    if math.floor(levels[identifier]) - level > 0 then
        TriggerClientEvent('avid-fishing:showNotification', player.source, locale('unlocked_level'), 'success')
    end

    TriggerClientEvent('avid-fishing:updateLevel', player.source, levels[identifier])
end

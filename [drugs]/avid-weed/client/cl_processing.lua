...
local placeProcessingTable = function(ped, tableItem, coords, rotation, metadata)
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:createNewTable', coords, tableItem, rotation, metadata)
...
RegisterNetEvent('avid-weed:client:placeProcessingTable', function(tableItem, metadata)
...
RegisterNetEvent('avid-weed:client:useTable', function(data)
    local netId = NetworkGetNetworkIdFromEntity(data.entity)
    lib.callback('avid-weed:server:getTableData', false, function(result)
        if not result then return end
        TriggerEvent('avid-weed:client:showRecipesMenu', result)
    end, netId)
end)

RegisterNetEvent('avid-weed:client:processDrugs', function(args)
    ...
    if Config.ProcessingSkillCheck then
        for i = 1, amount, 1 do
            ...
            if success then
                ...
                TriggerServerEvent('avid-weed:server:processDrugs', entity, recipe)
            else
                proccessing = false
...
RegisterNetEvent('avid-weed:client:removeTable', function(args)
    local entity = args.entity
    ...
    if lib.progressBar({
        ...
        TriggerServerEvent('avid-weed:server:removeTable', {entity = entity})
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else

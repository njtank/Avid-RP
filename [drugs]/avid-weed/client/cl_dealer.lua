...
CreateThread(function()
    for dealerID, dealerData in pairs(Config.DrugDealers) do
        ...
        local dealerPosition = lib.callback.await('avid-weed:server:getDealerPosition', false, dealerID)

        while not dealerPosition do
            Wait(500)
            dealerPosition = lib.callback.await('avid-weed:server:getDealerPosition', false, dealerID)
...
RegisterNetEvent('avid-weed:client:handelBuyInteraction', function(args)
    ...
    TriggerServerEvent('avid-weed:server:buyDealerItem', dealerId, item, amount, total)    
end)

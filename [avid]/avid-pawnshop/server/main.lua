    ...
    for shopId, data in pairs(Config.Shops) do
        if payload.toInventory == shopId then
                ...
                if payload.fromSlot.name == item then
                    if data.placeholders then
                        if payload.fromSlot.name ~= payload.toSlot.name then
                            TriggerClientEvent('avid-pawnshop:Notify', source, Strings.Notify.wrongSlot, 'inform')
                    ...
                    local result = lib.callback.await('avid-pawnshop:ConfirmSale', source, nil, shopId, item, price, quantity)
                    if not result then
                        EventLog('[main.lua]: BeginTransaction: sale was cancelled by player, cannot proceed..')
                        return false
                    end
                    TriggerClientEvent('avid-pawnshop:Notify', source, Strings.Notify.complete ..GroupDigits(price).. Strings.Notify.complete2 ..data.account.. Strings.Notify.complete3 ..GroupDigits(quantity).. ' ' ..tostring(info.label), 'success')
            ...
            if not itemAccepted then
                ...
                TriggerClientEvent('avid-pawnshop:Notify', source, Strings.Notify.cantSell, 'error')
                return false
            end
        end
    end

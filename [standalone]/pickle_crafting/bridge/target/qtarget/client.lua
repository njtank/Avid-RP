if GetResourceState('ox_target') == 'started' or GetResourceState('qtarget') ~= 'started' or not Config.EnableTarget then return end
...
function InitTarget()
    ...
    for k,v in pairs(Config.Tables) do 
        ...
        if not cfg.model then 
            exports['qtarget']:AddBoxZone("crafting_coord_" .. k, v.coords, 0.2, 0.2, {
                name = "crafting_coord_" .. k,
                heading = v.heading,
                minZ = v.coords.z,
                maxZ = v.coords.z + 0.2,
            }, {
